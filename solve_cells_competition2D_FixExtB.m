...
function [r,fe_r,fe_t,P,V,VT,R,radial,hoop,C,Gam,residual,LamM,LamA] = solve_cells_competition2D_FixExtB(param)
...
% p, v iteration
use_nonlinear = 0;
max_vp_ite = 1; % max iterations of v,p
tol = 1e-6;
...
%% nonlinear solver
    function F = fvp(x)
        ...
        % p tumor
        %F(Nr+1:2*Nr-1) = -1.5*x(Nr+1:2*Nr-1)+2*x(Nr+2:2*Nr)-0.5*x(Nr+3:2*Nr+1) ...
        %    + dx*scale_v*R(n-1)*x(1:Nr-1) - dx*fr(1:Nr-1);
        %F(2*Nr) = -0.5*x(2*Nr-1)+0.5*x(2*Nr+1) ...
        %    + dx*scale_v*R(n-1)*x(Nr) - dx*fr(Nr);
        %
        ...
        F(2*Nr+2) = -0.5.*x(2*Nr+1)+dx2/ratio_map./old_r(Nr+2).*x(2*Nr+2)+0.5.*x(2*Nr+3)-dx2/ratio_map*gamExt(2);
        F(2*Nr+3:2*Nr+Nrext) = 0.5*x(2*Nr+1:2*Nr+Nrext-2)-2*x(2*Nr+2:2*Nr+Nrext-1)+(1.5+dx2/ratio_map./old_r(Nr+3:Nrall)).*x(2*Nr+3:2*Nr+Nrext)...
            -dx2/ratio_map*gamExt(3:Nrext);
        %F(2*Nr+Nrext) = 0; %v=0 at r infinity
        %p exterior
        %F(2*Nr+Nrext+1) = x(2*Nr+Nrext+1)-x(2*Nr)-sr(Nr+1)+sr(Nr);
        %F(2*Nr+Nrext+2:2*Nrall-2) = -1.5*x(2*Nr+Nrext+2:2*Nrall-2)+2*x(2*Nr+Nrext+3:2*Nrall-1)-0.5*x(2*Nr+Nrext+4:2*Nrall) ...
        %    + dx2*scale_v_ext*(nrsize-R(n-1))./(nrsize-1)*x(2*Nr+2:2*Nr+Nrext-2) - dx2*fr(Nr+2:Nrall-2);
        F(2*Nr+Nrext+1:2*Nrall-2) = -1.5*x(2*Nr+Nrext+1:2*Nrall-2)+2*x(2*Nr+Nrext+2:2*Nrall-1)-0.5*x(2*Nr+Nrext+3:2*Nrall) ...
            + dx2*scale_v_ext*(nrsize-R(n-1))./(nrsize-1)*x(2*Nr+1:2*Nr+Nrext-2) - dx2*fr(Nr+1:Nrall-2);
        F(2*Nrall-1) = -0.5*x(2*Nrall-2)+0.5*x(2*Nrall) ...
            + dx2*scale_v_ext*(nrsize-R(n-1))./(nrsize-1)*x(Nrall+Nr-1) - dx2*fr(Nrall-1);
...
%% Growth Function
    function [Gamma,LamM,LamA] = get_Gamma_T(fer,fet,p)
        ...
        %LamM = param(4)*0.5*param(5)*ones(Nr,1);
        LamM = param(5)*(1+param(15).*abs(Sig_inv).*(Sig_inv>-tol2)./(1+param(15).*abs(Sig_inv).*(Sig_inv>-tol2)));
        %LamA = param(4).*param(19)*abs(Sig_inv).*(Sig_inv<tol2)./(1+0.1*abs(Sig_inv).*(Sig_inv<tol2));
        LamA = param(4).*param(19)*abs(Sig_inv).*(Sig_inv<tol2)./(1+param(19)*abs(Sig_inv).*(Sig_inv<tol2));
        %LamA = param(4)*(Ptil+W);
    ...
    function [Gamma,LamM,LamA] = get_Gamma_Ext(fer,fet,pext)
        ...
        LamM = param(14)*(param(20)*abs(Sig_inv).*(Sig_inv>-tol2)./(1+param(20)*abs(Sig_inv).*(Sig_inv>-tol2)));
        LamA = param(13).*param(21)*abs(Sig_inv).*(Sig_inv<tol2)./(1+param(21)*abs(Sig_inv).*(Sig_inv<tol2));
        %LamA = param(13).*param(21)*(Sig_inv).^2.*(Sig_inv<tol2)./(1+0.1*(Sig_inv).^2.*(Sig_inv<tol2));
        %LamM = param(13)*0.5*param(14)*ones(Nrext,1);
        %LamA = param(13).*(Ptil+Wext);
...
for n=nStart:Nt+1
    ...
    % end
    if R(n-1) >= nrsize
        disp('Tumor is bigger than the domain')
        break;
    end
    ...
    old_r = (r-1)*(nrsize-R(n-1))./(nrsize-1)+R(n-1);
    ratio_map = (nrsize-1)/(nrsize-R(n-1));
    ...
    fr(1) = sigma_r3(1);
    fr(Nr+1:Nrall) = sigma_r3(Nr+1:Nrall)+(sr(Nr+1:Nrall)-st(Nr+1:Nrall))./ratio_map./old_r(Nr+1:Nrall);
    ...
    diagVExt = [1; dx2/ratio_map/old_r(Nr+2); (1.5+dx2/ratio_map./(old_r(Nr+3:Nrall)))];
    subDiagVExt = [0;-0.5;-2*ones(length(Nr+3:Nrall),1)*1];
    subsubDiagVExt = [0.5*ones(length(Nr+3:Nrall),1)*1];
    ...
    VPcoupledExt = [ones(Nrext-1,1)*scale_v_ext*(nrsize-R(n-1))./(nrsize-1)*dx2;0]; % lower left submatrix. the p_r+R*v bit
    ...
    if ~use_nonlinear
        for i_vp=1:max_vp_ite % iterate a few times to eliminate oscillation
            ...
            rhsVExt = [0; dx2/ratio_map*gamExt(2:Nrext)];
            ...
            if any(isnan(sol)) || any(isinf(sol))
                save('dataNaNorInf.mat','sol','param','gamT','gamExt','fet','R')
                error('Error Inf Linear')
            end
            v = sol(1:Nr);
            p = sol(Nr+1:2*Nr);
    ...
    vtext = (nrsize-1)./(nrsize-R(n)).*(vext-drdt);
    ...
    fetnew(Nr+1:Nrall) = fet(Nr+1:Nrall)-dt*(max(vtext(1:Nrext),0).*Fet_r_mExt(1:Nrext)+min(vtext(1:Nrext),0).*Fet_r_pExt(1:Nrext))...
        +dt*(vext(1:Nrext)./((r(Nr+1:Nrall)-1)*(nrsize-R(n-1))/(nrsize-1)+R(n-1))-D2ext(1:Nrext)).*fet(Nr+1:Nrall);
